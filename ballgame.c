...
void update_ball(void){
	...
	if(balldrop == true){
		...
		if(current_level == 4){
			int size =  sizeof(level4coll) / sizeof(level4[0]);
			for (int i = 0; i < size; i++){
				if(collision(ballx, bally, level4coll[i].p1, level4coll[i].p2)){
					liveLost();
					break;
				}
			}
		...
		if(current_level == 5){
			int size =  sizeof(level5coll) / sizeof(level5coll[0]);
			for (int i = 0; i < size; i++){
				if(collision(ballx, bally, level5coll[i].p1, level5coll[i].p2)){
					liveLost();
					break;
				}
			}
...
void update_cursor(void){
	...
	if(current_level == 4){
		for(int i = 0; i < sizeof(level4coll)/sizeof(level4[0]); i++){
			if(collision(cursorx, cursory, level4coll[i].p1, level4coll[i].p2)){
				cursorx = cursorx-direction_array[0]-direction_array[3];
				cursory = cursory-direction_array[1]-direction_array[2];
			}
		}
	...
	if(current_level == 5){
		for(int i = 0; i < sizeof(level5coll)/sizeof(level5coll[0]); i++){
			if(collision(cursorx, cursory, level5coll[i].p1, level5coll[i].p2)){
				cursorx = cursorx-direction_array[0]-direction_array[3];
				cursory = cursory-direction_array[1]-direction_array[2];
			}
		}
